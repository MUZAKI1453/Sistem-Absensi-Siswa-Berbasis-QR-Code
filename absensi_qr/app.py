...
def create_qr_with_text(nis, nama):
    ...
    # Menggunakan draw.textbbox() untuk mendapatkan ukuran teks
...
def download_qr(nis):
    ...
    # Membuat gambar QR code dengan teks secara dinamis
    qr_image = create_qr_with_text(siswa_data.nis, siswa_data.nama)

    # Menyimpan gambar di memori (in-memory)
    img_io = io.BytesIO()
...
def update_absensi(nis):
    ...
    # PERBAIKAN: Mengarahkan kembali ke halaman absensi, bukan siswa
