...
def absensi():
    ...
    cari_nama = request.args.get("cari_nama")  # Tambahkan baris ini
    kelas_list = [k[0] for k in db.session.query(Siswa.kelas).distinct().order_by(Siswa.kelas).all()]

    # Mulai dengan query untuk semua siswa
    ...
    if kelas:
        siswa_query = siswa_query.filter(Siswa.kelas == kelas)
    ...
    # Ambil data siswa yang BELUM absen hari ini, berdasarkan query yang telah difilter
    siswa_belum_absen = siswa_query.filter(
        Siswa.nis.notin_(nis_sudah_absen)
    ).order_by(Siswa.nama.asc()).all()

    # Gabungkan daftar siswa
    semua_siswa_terurut = []
    # Urutkan siswa yang sudah absen sesuai filter
    for absen in absensi_masuk_terurut:
        siswa = siswa_query.filter_by(nis=absen.nis).first()
        if siswa:
            semua_siswa_terurut.append(siswa)
    semua_siswa_terurut.extend(siswa_belum_absen)

    # Buat dictionary data absensi
    data_absensi = {}
    ...
    return render_template(
        ...
        kelas=kelas,
        cari_nama=cari_nama  # Tambahkan variabel ini ke template
    )
...
def update_absensi(nis):
    ...
    kelas = request.form.get("kelas")  # Ambil kelas dari form
    cari_nama = request.form.get("cari_nama")  # Ambil nama dari form
    ...
    if status and nis:
        ...
        if existing_absensi:
            ...
            # Keterangan diperbarui hanya jika disediakan
            if keterangan:
    ...
    # Sertakan kembali parameter pencarian saat redirect
    return redirect(url_for("absensi", kelas=kelas, cari_nama=cari_nama))
