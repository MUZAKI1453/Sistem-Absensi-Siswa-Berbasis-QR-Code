...
def absensi():
    ...
    # 1. Ambil data absensi masuk untuk hari ini, diurutkan berdasarkan waktu (timestamp).
    absensi_masuk_terurut = Absensi.query.filter(
        Absensi.tanggal == hari_ini,
        Absensi.jenis_absen == "masuk"
    ).order_by(Absensi.waktu.asc()).all()

    # 2. Dapatkan NIS dari siswa yang sudah absen masuk, dalam urutan waktu.
    nis_sudah_absen = {absen.nis for absen in absensi_masuk_terurut}

    # 3. Ambil data siswa yang BELUM absen hari ini, diurutkan berdasarkan nama.
    siswa_belum_absen = Siswa.query.filter(
        Siswa.nis.notin_(nis_sudah_absen)
    ).order_by(Siswa.nama.asc()).all()

    # 4. Gabungkan daftar siswa: yang sudah absen duluan, diikuti yang belum absen.
    #    Ini akan menjadi dasar pengurutan dictionary kita.
    semua_siswa_terurut = [
                              Siswa.query.filter_by(nis=absen.nis).first() for absen in absensi_masuk_terurut
                          ] + siswa_belum_absen

    # 5. Buat dictionary yang akan dikirim ke template, dan pastikan
    #    urutan pemasukan data sesuai dengan urutan yang kita inginkan.
    data_absensi = {}

    # Ambil semua data absensi hari ini (masuk, pulang, lainnya) untuk melengkapi dictionary
    ...
    # Masukkan data ke dictionary yang terurut
    for siswa in semua_siswa_terurut:
        if siswa:
    ...
    # Terakhir, filter berdasarkan kelas jika ada
    if kelas:
        data_absensi_filtered = {
            nis: data for nis, data in data_absensi.items() if data['siswa'].kelas == kelas
        }
        data_absensi = data_absensi_filtered
...
def atur_waktu():
    """Mengelola pengaturan waktu untuk absensi masuk, pulang, dan keterlambatan."""
    auth_check = check_admin_session()
    if auth_check:
        return auth_check
    ...
    if request.method == "POST":
        ...
        # Simpan waktu terlambat jika ada
...
# Fungsi baru untuk mengambil data absensi harian dari database
